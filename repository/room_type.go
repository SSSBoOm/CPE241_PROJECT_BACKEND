...
func (r *roomTypeRepository) Update(roomType *domain.RoomType) error {
	...
	t := r.db.MustBegin()
	_, err := t.NamedExec("UPDATE room_type SET name = :name, detail = :detail, price = :price, accommodate = :accommodate, is_active = :is_active, update_at = :update_at WHERE id = :id", roomType)
	if err != nil {
		t.Rollback()
		return err
	}
