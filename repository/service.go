...
func (r *serviceRepository) Create(service *domain.SERVICE) error {
	t := r.db.MustBegin()
	_, err := t.NamedExec("INSERT INTO service (name, is_active, service_type_id) VALUES (:name, :is_active, :service_type_id)", service)
	if err != nil {
		t.Rollback()
		return err
	}
...
func (r *serviceRepository) Update(service *domain.SERVICE) error {
	...
	t := r.db.MustBegin()
	_, err := t.NamedExec("UPDATE service SET name = :name, is_active = :is_active, service_type_id = :service_type_id, updated_at = :updated_at WHERE id = :id", service)
	if err != nil {
		t.Rollback()
		return err
	}
